...
vim.api.nvim_create_autocmd({ "BufWritePost" }, {
    ...
    callback = function()
        local command = 'golangci-lint run ./' ..
            vim.fn.fnamemodify(vim.fn.expand('%:h'), ':p:~:.') .. '--fix --out-format json'
...
vim.keymap.set("n", "<F6>",
    '<cmd>lua Run_cmd("go test -v -race ./"..vim.fn.fnamemodify(vim.fn.expand("%:h"), ":p:~:."))<cr>', keyOpts) -- TODO: do not run when not test file
vim.keymap.set("n", "<F7>", '<cmd>lua Run_cmd("go test -v -race ./...")<cr>', keyOpts)
